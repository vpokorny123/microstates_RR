...
% Paths to the data
allsubsDir2load = '/labs/srslab/data_staging/DEFEND_archive/';
%% since we have a bunch of groups let's just do everyone together and sort it out on the backend
...
d_Fs = 128;                             % desired frequency in Hz
...
%for iGroup = 1:size(Group,1)
    ...
    for iSubject = 1   %:size(Subject_pool,1)  %just running for one person is porlly good enough
        
        % Get the subject
        bdf_name = [char(Subject_pool{iSubject}), '_RestingEEG.bdf'];
        bdfFile = fullfile(Subject_dir,Subject_pool{iSubject},bdf_name);
        ...
        dataset = pop_biosig(bdfFile, 'ref',[129 130] ,'refoptions',{'keepref' 'off'}, 'importannot','off');
        dataset.data2interp = dataset.data(1:128,:);
        [interpEEG] = convert_128to64(dataset);
        %dataset = pop_biosig(bdfFile, 'ref',[129 130] ,'refoptions',{'keepref' 'off'}, 'importannot','off');
        dataset.data = [interpEEG;dataset.data([131:138],:)]; %skipping ears cuz we don't need them  don't think
        dataset.nbchan = size(dataset.data,1);
        ...
        %heog = dataset.data(nch+,:) - dataset.data(134,:); % HEOG channel (left-right - looking at the subj)
        %veog = dataset.data(135,:) - dataset.data(136,:); % VEOG channel (top - bottom)
...
%         cd(strcat(SaveDir,'\',char(Group{iGroup})));
        ...
        save(strcat(SaveDir,char(Subject_pool{iSubject}),'_preprocess.mat'),'EEG',...
